...
enum CodeMapExtractor {
    ...
    private static func appendMCPTruncationNotice(to text: inout String) {
        ...
        static func generateFileTreeForSubsetFiles(
            ...
            guard !rootFolders.isEmpty else { return "" }
            let subsetList = Array(subsetFullPaths)
            return generateFileTreeForSubsetPaths(
                rootFolders: rootFolders,
                subsetPaths: subsetList,
                filePathDisplay: filePathDisplay,
                includeLegend: includeLegend,
                codeMapAvailableFullPaths: codeMapAvailableFullPaths,
                showCodeMapMarkers: showCodeMapMarkers
        ...
        /// Builds a file tree limited to a subset of paths (files or subfolders) in the workspace.
        static func generateFileTreeForSubsetPaths(
            rootFolders: [FolderViewModel],
            subsetPaths: [String],
            filePathDisplay: FilePathDisplay,
            ...
            guard !rootFolders.isEmpty, !subsetPaths.isEmpty else { return "" }
            if Task.isCancelled { return "" }

            // Expand starting paths: if /a/b/c is selected, then /a and /a/b are implicit internal nodes.
            var visiblePaths = Set<String>()
            for path in subsetPaths {
                if Task.isCancelled { break }
                let stdPath = StandardizedPath.absolute(path)
                visiblePaths.insert(stdPath)

                // Walk upwards to the matching root folder.
                var parent = (stdPath as NSString).deletingLastPathComponent
                while !parent.isEmpty, parent != "/" {
                    if Task.isCancelled { break }
                    let stdParent = StandardizedPath.absolute(parent)
                    // Stop climbing once we match a root or cross boundary
                    let isAncestorOfRoot = rootFolders.contains { root in
                        let stdRoot = root.standardizedFullPath
                        return stdRoot == stdParent || stdRoot.hasPrefix(stdParent + "/")
                    }
                    if isAncestorOfRoot { break }

                    visiblePaths.insert(stdParent)
                    parent = (parent as NSString).deletingLastPathComponent
                }
            }

            var parts: [String] = []
            var usedCodeMapMarker = false

            for root in rootFolders {
                if Task.isCancelled { break }
                let stdRoot = root.standardizedFullPath

                // If a root contains no items in our visiblePaths, skip it.
                let hasVisibleDescendants = visiblePaths.contains { path in
                    path == stdRoot || path.hasPrefix(stdRoot + "/")
                }
                guard hasVisibleDescendants else { continue }

                let label = contextualRootLabel(for: root, within: FileTreeRenderedRootIdentity(root: root), filePathDisplay: filePathDisplay)
                var sb = StringBuilder()
                sb.appendLine(label)

                let outcome = buildSubtreeSubset(
                    folder: root,
                    currentDepth: 1,
                    parentPrefix: "",
                    isLastSibling: true,
            ...
            if includeLegend, usedCodeMapMarker, showCodeMapMarkers {
                output = codeMapLegend + "\n\n" + output
            }
            return output
        }

        private static func buildSubtreeSubset(
            ...
            showCodeMapMarkers: Bool,
            sb: inout StringBuilder,
            usedCodeMapMarker: inout Bool
        ) -> BuildOutcome {
            if Task.isCancelled { return .tooLarge }

            // Filter and collect children that are present in visiblePaths
            var visibleChildren: [FileSystemItemType] = []
            for child in folder.children {
                let childPath: String = switch child {
                case let .folder(sub): sub.standardizedFullPath
                case let .file(file): file.standardizedFullPath
                }
                if visiblePaths.contains(childPath) {
                    visibleChildren.append(child)
                }
            }

            guard !visibleChildren.isEmpty else { return .ok }

            // Render each visible child
            let nextPrefix = parentPrefix + (isLastSibling ? "    " : "│   ")
            for (index, child) in visibleChildren.enumerated() {
                ...
                switch child {
                case let .file(file):
                    let hasCodeMap = showCodeMapMarkers && codeMapAvailableFullPaths.contains(file.standardizedFullPath)
                    if hasCodeMap {
                        usedCodeMapMarker = true
                    }
                    let marker = hasCodeMap ? codeMapMark : ""
                    let line = parentPrefix + connector + file.name + marker
                    sb.appendLine(line)

                case let .folder(subfolder):
                    let line = parentPrefix + connector + subfolder.name
                    sb.appendLine(line)
        ...
        ) -> (tree: String, usedSelectedMarker: Bool, usedCodeMapMarker: Bool, wasTruncated: Bool) {
            if roots.isEmpty { return ("", false, false, false) }
            if Task.isCancelled { return ("", false, false, true) }

            var sb = StringBuilder(reserve: 4096)
            var usedSelectedMarker = false
            var usedCodeMapMarker = false

            // Determine if we are performing a budget-capped build
            if let budget = context.tokenBudget {
                let outcome = buildDepthZeroTree(
                    roots: roots,
                    settings: context.settings,
                    selectedFileIDs: context.selectedFileIDs,
                    fetchSelectedFolderIDs: context.getSelectedFolderIDs,
                    childrenCache: &context.childrenCache,
                    sb: &sb,
                    usedSelectedMarker: &usedSelectedMarker,
                    tokenBudget: budget
                )
                if outcome == .tooLarge {
                ...
                let m = context.settings.mode.lowercased()
                if m == "auto" {
                    // Progressive expansion until budget is exhausted or all elements fit.
                    let (tree, usedSelection, usedCodeMap, truncated) = generateFileTreeAutoWithBudget(
                        roots: roots,
                        tokenBudget: budget,
                        selectedFileIDs: context.selectedFileIDs,
                        selectedFolderIDs: context.getSelectedFolderIDs(),
                        showCodeMapMarkers: context.settings.showCodeMapMarkers
                    )
                    return (tree, usedSelection, usedCodeMap, truncated)
                } else {
                    return (sb.result, usedSelectedMarker, usedCodeMapMarker, false)
                }
            } else {
                // Full un-budgeted tree expansion (supports maxDepth filter)
                let outcome = buildDepthZeroTree(
                    roots: roots,
                    settings: context.settings,
                    selectedFileIDs: context.selectedFileIDs,
                ...
                return (sb.result, usedSelectedMarker, usedCodeMapMarker, outcome == .tooLarge)
            }
        }
        ...
        private static func generateFileTreeWithDepth(
            ...
            let settings = BuildSettings(
                mode: mode,
                filePathDisplay: showFullPaths ? .full : .relative,
                siblingCap: siblingCap,
                showCodeMapMarkers: showCodeMapMarkers
            )
            let getSelectedFolderIDs = { selectedFolderIDs }
        ...
        static func generateFileTreeStartingAtPath(
            ...
            let lookup = lookupFolderByPath(startPath, in: roots)
            guard let target = lookup else {
                return ("", false, false, false)
            }

            let m = context.settings.mode.lowercased()
            let folderIDs = context.getSelectedFolderIDs()

            // Build starting header line
            var sb = StringBuilder(reserve: 4096)
            let rootIdentity = FileTreeRenderedRootIdentity(root: target.root)
            let rootLabel = contextualRootLabel(for: target.folder, within: rootIdentity, filePathDisplay: context.settings.filePathDisplay)
            sb.appendLine(rootLabel)

            var usedSelectedMarker = false
            if folderIDs.contains(target.folder.id) {
                usedSelectedMarker = true
            }

            let cacheKey = VCCacheKey(folderID: target.folder.id, mode: m == "folders" ? 2 : 1)
            let items = localVisibleChildren(of: target.folder, mode: m, selectedFileIDs: context.selectedFileIDs, cache: &context.childrenCache)
            context.childrenCache[cacheKey] = items

            let outcome = buildSubtree(
                items: items,
                currentDepth: 1,
            ...
            return (sb.result, usedSelectedMarker, false, outcome == .tooLarge)
        }

        /// DFS lookup of a FolderViewModel by absolute path, preserving the top-level root that contains it.
        static func lookupFolderByPath(
            _ path: String,
            in roots: [FolderViewModel]
        ) -> (folder: FolderViewModel, root: FolderViewModel)? {
            let stdPath = StandardizedPath.absolute(path)
                ...
                while let current = stack.popLast() {
                    ...
                    for child in current.children {
                        if case let .folder(subfolder) = child {
                            let stdSub = subfolder.standardizedFullPath
                            if stdSub == stdPath {
                                return (subfolder, root)
                            }
                            if stdPath.hasPrefix(stdSub + "/") {
        ...
        private static func generateFileTreeForRoots(
            roots: [FolderViewModel],
            selectedFileIDs: Set<UUID>,
            selectedFolderIDs: Set<UUID>,
            showCodeMapMarkers: Bool
